"""
Transformer训练脚本 - SRU Dataset (Feature 1 & 3)
"""
import os
import sys
...
def train_epoch(model, train_loader, optimizer, device):
    ...
    for batch_data in pbar:
        ...
        # Forward
        predicted_data = model(masked_data, mask)
...
def main():
    print("="*70)
    print("Training Transformer Model - SRU Dataset")
    ...
    with open(os.path.join(config.save_dir, 'transformer_training_log.json'), 'w') as f:
        json.dump(log_data, f, indent=2)

    print(f"\nTraining completed! Best val loss: {best_val_loss:.6f}")
    print(f"Model saved: {config.save_dir}/transformer_best.pth")
