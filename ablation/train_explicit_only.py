...
def train_model():
    config = Config()
    save_dir = os.path.join(config.save_dir, 'explicit_only')
    os.makedirs(save_dir, exist_ok=True)

    print("=" * 60)
    print("消融实验: 仅显式特征扩散模型")
    print("=" * 60)
    ...
    print("\nCreating ExplicitOnlyDiffusionModel...")
    model = ExplicitOnlyDiffusionModel(
        input_dim=num_features,
        hidden_dim=config.hidden_dim,
        embedding_dim=config.embedding_dim,
        num_residual_layers=config.num_residual_layers,
        dropout=config.dropout,
        num_s4_layers=4  # 增加S4层数以补偿移除的隐式模块
    ).to(config.device)
    ...
    for epoch in range(config.num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
                'scaler': scaler
            }, os.path.join(save_dir, 'explicit_only_best.pth'))
    ...
    print(f"Model saved to: {os.path.join(save_dir, 'explicit_only_best.pth')}")
